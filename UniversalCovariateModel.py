...
feat_select_testing = True  # true when developing/adjusting feature selection method/data being used
...
new_logReg = True
auto_tune = True
covariate = 'Cardiovascular'
covariate_type = 'high blood pressure'  # ['Type 1 Diabetes', 'Type 2 Diabetes', 'Prediabetes', 'pre diabetes]
percent_variance = 3

'''Data Section'''

og_covariate = pd.read_csv('data/PatientMetaData_090722.csv', encoding='cp1252').set_index('PatientID').astype(str)[covariate]
dPath = 'C:/Users/jack/PycharmProjects/TruDiagnostic/Chronological Age/'
print('Total number of patient covariate data entries: ', len(og_covariate.index))
print(np.unique(og_covariate))

og_covariate.sort_index(0, inplace=True)
og_covariate.dropna(inplace=True)
...
if new_feature_select:
    ...
    if normalize_distrib:
        read_file = 'data/selected data/SelectedMethylationData' + covariate_type + '.csv'
    else:
        read_file = 'data/selected data/SelectedMethylationData' + covariate_type + '.csv'
...
relevant_cpgs = pd.read_csv('data/selected data/' + covariate_type + 'RelevantCpGs.csv')['Unnamed: 0']
og_betas = og_betas[relevant_cpgs]
og_betas.to_csv('data/Selected data/SelectedMethylationDta' + covariate_type + ' NEW.csv')
...
'''Removing all patients not shared between covariate Status data and methylation data'''
all_patients = og_covariate.index
beta_patients = []
missing_patients = []
for patient in all_patients:
    if patient in og_betas.index:
        beta_patients.append(patient)
        if covariate_type.lower() in str(og_covariate.loc[patient]).lower():
            og_betas.loc[patient, covariate_type + ' Status'] = 1
        else:
            og_betas.loc[patient, covariate_type + ' Status'] = 0
    else:
        missing_patients.append(patient)

comite_file = 'data/Comite_GMQN_BMIQ_betas.csv'
comite_betas = pd.read_csv(comite_file).set_index('Unnamed: 0')
for patient in comite_betas.index:
    if patient in og_covariate.index:
        if covariate_type.lower() in str(og_covariate.loc[patient]).lower():
            comite_betas.loc[patient, covariate_type + ' Status'] = 1
        else:
            comite_betas.loc[patient, covariate_type + ' Status'] = 0

dasatnib_file = 'data/DasatinibQuercitin_GMQN_Blood_Betas.csv'
dasatnib_betas = pd.read_csv(dasatnib_file).set_index('Unnamed: 0')
for patient in dasatnib_betas.index:
    if patient in og_covariate.index:
        if covariate_type.lower() in str(og_covariate.loc[patient]).lower():
            dasatnib_betas.loc[patient, covariate_type + ' Status'] = 1
        else:
            dasatnib_betas.loc[patient, covariate_type + ' Status'] = 0

s199_file = 'data/Sample250Test_betas.GMQN.csv'
s199_betas = pd.read_csv(s199_file).set_index('Unnamed: 0')
for patient in s199_betas.index:
    if patient in og_covariate.index:
        if covariate_type.lower() in str(og_covariate.loc[patient]).lower():
            s199_betas.loc[patient, covariate_type + ' Status'] = 1
        else:
            s199_betas.loc[patient, covariate_type + ' Status'] = 0
...
if new_feature_select:
    ...
    importances = mutual_info_classif(og_betas.drop(columns=[covariate_type + ' Status']), og_betas[covariate_type + ' Status'].astype(str))
    ...
    relevant_cpgs.to_csv('data/selected data/' + covariate_type + 'RelevantCpGs.csv')

    cpgs_target = list(relevant_cpgs.index)
    cpgs_target.append(covariate_type + ' Status')
    og_betas = og_betas[cpgs_target]
    og_betas.to_csv('data/selected data/SelectedMethylationData' + covariate_type + '.csv')
    print('Final Number of CpGs: ', len(cpgs_target))
elif feat_select_testing:
    relevant_cpgs = pd.read_csv('data/selected data/' + covariate_type + 'RelevantCpGs.csv')['Unnamed: 0'].to_list()
    cpgs_target = []
    ...
    threshold = average_importance * 3
    relevant_cpgs = feat_importances[feat_importances > threshold]
    relevant_cpgs.to_csv('data/selected data/' + covariate_type + 'RelevantCpGs2.csv')

    cpgs_target = list(relevant_cpgs.index)
    cpgs_target.append(covariate_type + ' Status')
    og_betas = og_betas[cpgs_target]
    og_betas.to_csv('data/selected data/SelectedMethylationData' + covariate_type + ' NEW.csv')
...
def create_duplicate(to_replicate, pct_variance):
    ...
    features.remove(covariate_type + ' Status')
...
if normalize_distrib:
    ...
    # Defining distribution of each Covariate Status
    if list(og_betas[covariate_type + ' Status']).count(1) > list(og_betas[covariate_type + ' Status']).count(0):
        to_upsample = 0
    else:
        to_upsample = 1

    df_duplicate = og_betas[og_betas[covariate_type + ' Status'] == to_upsample]
    pre_distribution = len(df_duplicate) / len(og_betas.index)

    # Add variable here
    print('Distribution Covariate Status before upsampling: \n', pre_distribution)
    upsample = 2
    ...
    post_distrib = len(og_betas[og_betas[covariate_type + ' Status'] == to_upsample]) / len(og_betas.index)

    print('Final distribution: ', post_distrib)

    og_betas.to_csv('data/Selected Data/Upsampled' + str(percent_variance) + '% NEW.csv')
else:
    post_distrib = len(og_betas[og_betas[covariate_type + ' Status'] == 1]) / len(og_betas.index)
    print('Distribution of each positive ' + covariate_type + ' factor: \n', post_distrib*100, '%')
...
train_x_labeled, test_x_labeled, train_y_labeled, test_y_labeled = train_test_split(og_betas.drop(columns=[covariate_type + ' Status']), og_betas[covariate_type + ' Status'], test_size=test_val_size)
...
dasat_x, dasat_y = scaler.transform(dasatnib_betas.drop(columns=[covariate_type + ' Status'])), np.array(dasatnib_betas[covariate_type + ' Status'])
comite_x, comite_y = scaler.transform(comite_betas.drop(columns=[covariate_type + ' Status'])), np.array(comite_betas[covariate_type + ' Status'])
s199_x, s199_y = scaler.transform(s199_betas.drop(columns=[covariate_type + ' Status'])), np.array(s199_betas[covariate_type + ' Status'])

joblib.dump(scaler, covariate_type + 'Scaler.joblib')
...
'''Model'''
if load_model:
    # model = keras.models.load_model('models/' + covariate_type + 'Predictor' + str(percent_variance) + '%')
    model = keras.models.load_model('models/' + covariate_type + 'Predictor0.01-0.01-10-100-5-0.15-0-0-0.1')
    validation_output = model.predict(val_x)
else:
    if auto_tune:
        space = {'min_lr': hp.uniform('min_lr', .0001, .01),
        ...
        def tuning_objective(hyperparameters={}):
            ...
            mod.save('D:/ftp/jackraymond/' + file_path)
            loss = mod.evaluate(dasat_x, dasat_y)[0]
                                        ...
                                        for b1 in b1s:
                                            for b2 in b2s:
                                                for dropout_rate in dropout_rates:
                                                    model_name = '' + covariate_type + 'Predictor' + str(learning_rate) + '-' + str(min_lr)\
                                                                 + '-' + str(patience) + '-' + str(layer_size) + '-' + \
                                                                 str(num_layers) + '-' + str(noise) + '-' + str(l1) + '-' \
                                                                 + str(l2) + '-' + str(dropout_rate)
                                                    print(model_name)
